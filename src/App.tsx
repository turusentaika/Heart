...
export default function App() {
  const [stage, setStage] = useState<'intro' | 'console' | 'reveal'>('intro');
  ...
  return (
    <div 
      ...
      className={`relative min-h-screen w-full flex items-center justify-center selection:bg-pink-deep/30 ${stage === 'console' && consoleFinished ? 'cursor-pointer' : ''}`}
    >
      <div className="scanline" />
      
      <AnimatePresence mode="wait">
        {stage === 'intro' ? (
          <IntroScene onUnlock={() => setStage('console')} />
        ) : stage === 'console' ? (
          <motion.div
            key="console"
            initial={{ opacity: 0 }}
          ...
          >
            <div className="space-y-2">
              <div className="flex gap-2 text-pink-soft/60">
                <span>[system]</span>
                <Typewriter 
                  text="Initializing heart.PROTOCOL_v3.0..." 
                  delay={30} 
              ...
              {consoleFinished && (
                ...
                >
                  ...
                  <button
                    ...
                    className="group flex items-center gap-3 px-6 py-3 border border-pink-deep/30 bg-pink-deep/5 hover:bg-pink-deep/10 text-pink-soft transition-all duration-300 pointer-events-auto hover:scale-105 hover:-translate-y-0.5 hover:shadow-lg hover:border-pink-deep/60 cursor-pointer"
                  >
